...
export function useChatRequest(requestId: string | undefined, pollIntervalMs = 2_500) {
  ...
  const refresh = useCallback(async () => { if (!requestId) return; try { setErrorCode(''); setChat(await getChatRequest(requestId)) } catch (error) { setErrorCode(error instanceof Error ? error.message : 'NETWORK_ERROR') } finally { setIsLoading(false) } }, [requestId])
