...
export function LoginPage() {
  ...
  return <><p className="eyebrow">ENTERPRISE SECURITY SERVICE</p><h1>Secure AI Console</h1><p>기관 계정으로 로그인해 안전한 AI 서비스를 이용하세요.</p><Alert variant="info" title="기관 자동 매핑">개발 환경에서는 테스트 기관으로 자동 연결됩니다. 운영 환경에서는 인증 서버의 기관 매핑 결과를 사용합니다.</Alert><form onSubmit={handleSubmit}><FormField label="사용자 ID"><TextInput value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="off" required /></FormField><FormField label="비밀번호"><TextInput type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="off" required /></FormField>{error && <Alert variant="danger" title="로그인 실패">{error}</Alert>}<Button type="submit" disabled={isSubmitting}>{isSubmitting ? '로그인 중' : '로그인'}</Button></form><Alert variant="warning" title="보안 안내">비밀번호와 인증정보는 브라우저 저장소에 저장하지 않습니다.</Alert><small>개발용 Mock 인증입니다. 운영 환경에서는 승인된 인증 서버를 연결해야 합니다.</small></>
}
